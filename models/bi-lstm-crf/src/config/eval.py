...
def evaluate_batch_insts(batch_insts: List[Instance],
    ...
    for idx in range(len(batch_pred_ids)):
        ...
        start = -1
        for i in range(len(output)):
            if output[i].startswith("B-") and i == len(output) - 1:
                output_spans.add(Span(i, i, output[i][2:]))
                batch_total_entity_dict[output[i][2:]] += 1
            if output[i].startswith("B-") and i != len(output) -1:
                start = i
                label = output[i][2:]
                next = i + 1
                while next < len(output) and label == output[next][2:]:
                    next += 1
                end = next - 1
                output_spans.add(Span(start, end, output[i][2:]))
                batch_total_entity_dict[output[i][2:]] += 1
        ...
        start = -1
        for i in range(len(prediction)):
            if prediction[i].startswith("B-") and i == len(prediction) - 1:
                predict_spans.add(Span(i, i, prediction[i][2:]))
                batch_total_predict_dict[prediction[i][2:]] += 1
            if prediction[i].startswith("B-") and i != len(prediction) - 1:
                start = i
                label = prediction[i][2:]
                next = i + 1
                while next < len(prediction) and label == prediction[next][2:]:
                    next += 1
                end = next - 1
                predict_spans.add(Span(start, end, prediction[i][2:]))
                batch_total_predict_dict[prediction[i][2:]] += 1
