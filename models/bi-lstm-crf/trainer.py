...
import wandb
wandb.init(project="blstm_test", name="lstm", config={"model_name": "lstm"})
...
def parse_arguments(parser):
    ...
    parser.add_argument('--add_iobes_constraint', type=int, choices=[0, 1], default=1,
...
def train_model(config: Config, epoch: int, train_loader: DataLoader, dev_loader: DataLoader, test_loader: DataLoader):
    ...
    for i in tqdm(range(1, epoch + 1), desc="Epoch"):
        ...
        wandb.log({
            "train/loss": float(format(epoch_loss / len(train_loader), ".4f")),
            "train_runtime": float(format(train_runtime, ".3f")),
            "train_runtime per Epoch ": float(format((end_time - start_time), ".3f")),
            "train_samples_per_second ": float(format(len(train_loader.dataset) / (end_time - start_time), ".3f")),
        })
        torch.save(model.state_dict(), f"model_files/{model_folder}/lstm_crf_Epoch_{i}.m")
        f = open(f"model_files/{model_folder}/config_Epoch_{i}.conf", 'wb')
        pickle.dump(config, f)

        model.eval()
        dev_metrics = evaluate_model(config, model, dev_loader, "dev", dev_loader.dataset.insts)
        wandb.log({
            "eval_precision": dev_metrics[0] / 100,
            "eval_recall": dev_metrics[1] / 100,
            "eval_fscore": dev_metrics[2] / 100,
            "eval_loss": dev_metrics[3],
        })
...
def evaluate_model(config: Config, model: NNCRF, loader: DataLoader, name: str, insts: List[Instance], print_each_type_metric: bool = True):
    ...
    with torch.no_grad():
        ...
        for iter, batch in tqdm(enumerate(loader, 1), desc="--evaluating batch", total=len(loader)):
            ...
            batch_max_scores, batch_max_ids, batch_loss = model.decode(words = batch.words.to(dev), word_seq_lens = batch.word_seq_len.to(dev),
                         context_emb=batch.context_emb.to(dev) if batch.context_emb is not None else None,
                         chars = batch.chars.to(dev), char_seq_lens = batch.char_seq_lens.to(dev),  labels = batch.labels.to(dev))
    ...
    return [precision, recall, fscore, float(format(epoch_loss/len(loader), ".4f"))]
